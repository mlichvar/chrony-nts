...
*nts*:::
This option enables authentication with the Network Time Security (NTS)
protocol. It is supported only in the *server* and *pool* directives.
*maxdelay* _delay_:::
...
[[ntscacert]]*ntscacert* _file_::
This directive specifies a file containing certificates of trusted
CAs that should be used to verity certificates of NTS servers in addition to
the system's default trusted CAs.
...
Key Establishment (NTS-KE) service as an NTS server. The default is 11443.
+
...
This directive specifies a certificate for *chronyd* to operate as an NTS
server.

[[ntsserverkey]]*ntsserverkey* _file_::
This directive specifies a private key for *chronyd* to operate as an NTS
server.
